...
----------------------------------------------------------------------------------------------------
| Plik:             Dijkstra.cpp
| Opis:             Implementacja  algorytmu Dijkstry.
| Autor:            Dawid Janik
...
#pragma omp for schedule(dynamic,4)private(i)	
	for (int i = 0; i < graph->Vertices[s].Quantity; i++)
	{
...
int* Dijkstra::FindShortestWay()
{
	...
	while (m_Q > 0) // dopoki nie sa obliczone wszystkie wierzcholki
	{
		...
		for (int i = 0; i < m_Graph.Vertices[counted].Quantity; i++) // sprawdz krawedzie wierzcholka i 
		{
...
void Dijkstra::ShowJson()
{
	string tmp;
	for (int i = 0; i < m_Graph.NumberOfVertices; i++) {
		cout << i << ":" << m_D[i];
		if (i < m_Graph.NumberOfVertices - 1)
			cout << ",";
	}
}